...
# Becoming Datakeeper: Node Setup Instruction
...
- Open [https://peaq.subscan.io/account/YOUR_ADDRESS](https://peaq.subscan.io/account/YOUR_ADDRESS)
...
![Executables](assets/executables.png)
...
1. Open Terminal: Press **Win + R**, type `cmd` or `powershell`, and press Enter.
   ...
   ![Windows Command Prompt](assets/win-cmd.png)
2. Start the Application:
    - Navigate to the Application Folder

      ![Windows Folder](assets/win-folder.png)
    - Run application executable

      ![Windows Run](assets/win-run.png)
...
2. Run the following commands to create folder, copy, and run denode:
   ...
   ![Linux Run](assets/linux-run.png)
...
8. **Verify Operation**:
    - Watch the terminal output. If no errors appear, your DeNet Node is running correctly.
      ...
      ![Successful Launch](assets/successful-launch.png)

### Step 5: Monitor Transactions
Track your node’s activity using the peaq Subscan web interface.
1. Visit the peaq Subscan website (e.g., [https://peaq.subscan.io/account/YOUR_ADDRESS](https://peaq.subscan.io/account/YOUR_ADDRESS)).
...
### Troubleshooting
- **Errors in Terminal**: Carefully check error message. Most errors are related to the lack of Internet, insufficient balance of gas tokens, or the result of manually changing the data generated by the node. Ask for help from community members or contact support in Discord.
- **Failed Transactions in Subscan**: There may be some unusual situations where transactions fail. If you encounter such cases, please open a support ticket.
  ...
  ![Failed Transaction](assets/failed-trx.png)
- **Port Conflicts**: If port 55050 is in use, try another port (e.g., 55051)

  ![Port Error](assets/port-error.png)
...
- **Not Opened macOS**:
    - Run the following command to allow `denode` executable:
      ...
      ![macOS Error](assets/mac-error-01.png)
- **Permission Denied**:
  ```bash
...
A graphical user interface (GUI) for seamless node operation is coming soon. Stay tuned!

#### Systemd Service (Linux)
- It is recommended to run as a non-root user.
...
1. Remove old credentials and configurations, they should be imported from scratch:
   ...
   ![Windows Remove Denode](assets/win-rm-denode.png)
2. For **Linux/macOS**: Download installation and management scripts from the [scripts](https://github.com/DeNetPRO/Node/tree/Dev/scripts) directory:
   ```shell
   install.sh
   denode-manager.sh
   ```
...
1. Download archive for your system from [https://github.com/DeNetPRO/Node/releases](https://github.com/DeNetPRO/Node/releases) as well as for [denode](#step-2-download-datakeeper-node)
   #### Windows
   ```
...
1. Open browser and go to [http://localhost:1111](http://localhost:1111)
...
2. You shouldn't use both CLI and GUI at the same time, otherwise you will get undefined application behavior.
3. We recommend changing the RPC to a private one immediately after the node launch. This will help avoid problems with default version limitations.
